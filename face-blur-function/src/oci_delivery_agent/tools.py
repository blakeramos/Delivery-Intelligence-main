...
try:  # pragma: no cover - optional dependency for face detection
    import cv2
    CV2_AVAILABLE = True
except Exception:  # pragma: no cover - fall back if opencv not installed
    cv2 = None
    CV2_AVAILABLE = False
...
class ObjectStorageClient:
    ...
    def __init__(self, config: WorkflowConfig):
        ...
        self._namespace = None  # Cache namespace for put operations
    ...
    def put_object(self, object_name: str, data: bytes, content_type: str = "image/jpeg") -> Dict[str, Any]:
        """
        Upload object to OCI Object Storage or save locally.
        ...
        # Try OCI upload first if client exists
        if (self._client is not None and 
            self._config.object_storage.namespace != "test" and 
            self._config.object_storage.bucket_name != "test"):  # pragma: no cover
            try:
                ...
                return {
                    "object_name": resolved_name,
                    "namespace": self._config.object_storage.namespace,
                    "bucket": self._config.object_storage.bucket_name,
                    "size": len(data),
                    "uploaded_at": datetime.utcnow().isoformat(),
                    "storage_path": f"oci://{self._config.object_storage.namespace}/{self._config.object_storage.bucket_name}/{resolved_name}",
                    "source": "oci"
                }
            except Exception as e:
                print(f"Failed to upload to OCI, falling back to local: {e}")
        ...
        return {
            ...
            "size": len(data),
            "uploaded_at": datetime.utcnow().isoformat(),
            "storage_path": f"file://{output_path.absolute()}",
            "source": "local"
        }
...
def blur_faces_in_image(
    ...
    """
    Detect and blur human faces in an image to protect privacy.
    
    Args:
        image_bytes: Raw image bytes (JPEG, PNG, etc.)
        blur_intensity: Gaussian blur kernel size (must be odd). Higher = more blur.
                       Default 51 provides strong anonymization.
        scale_factor: How much the image size is reduced at each image scale.
                     Lower values (e.g., 1.05) are more thorough but slower.
        min_neighbors: How many neighbors each candidate rectangle should have
                      to retain it. Higher values result in fewer false positives.
        min_face_size: Minimum possible face size (width, height) in pixels.
    ...
    if not CV2_AVAILABLE or cv2 is None:
        raise RuntimeError(
            "OpenCV (cv2) is required for face blurring. "
    ...
    try:
        ...
        # This classifier comes bundled with OpenCV
        ...
        for (x, y, w, h) in faces:
            ...
            # Use minimum 40% of face dimension for effective anonymization
            face_size = max(w, h)
            ...
            adaptive_blur = min(adaptive_blur, 299)
        ...
        if save_format == 'JPEG':
            pil_image.save(output_buffer, format=save_format, quality=95)
        else:
        ...
        raise ValueError(f"Failed to process image for face blurring: {e}")


class FaceBlurringTool(BaseTool):
    """LangChain tool for detecting and blurring faces in images."""
    
    name: str = "blur_faces"
    description: str = (
        "Detect and blur human faces in delivery photos to protect privacy. "
        "Returns the processed image with all detected faces anonymized using Gaussian blur."
    )
    blur_intensity: int = 51  # Pydantic field
    scale_factor: float = 1.05  # Pydantic field
    min_neighbors: int = 3  # Pydantic field
    min_face_size: Tuple[int, int] = (20, 20)  # Pydantic field
    
    def __init__(self, blur_intensity: int = 51, scale_factor: float = 1.05, 
                 min_neighbors: int = 3, min_face_size: Tuple[int, int] = (20, 20)):
        """
        Initialize the face blurring tool.
        
        Args:
            blur_intensity: Gaussian blur kernel size (must be odd). Default 51.
            scale_factor: Detection scale factor. Lower = more thorough. Default 1.05.
            min_neighbors: Detection sensitivity. Lower = more sensitive. Default 3.
            min_face_size: Minimum face size to detect. Default (20, 20).
        """
        super().__init__(
    ...
    def _run(self, encoded_payload: str) -> str:
        """
        Blur faces in a base64-encoded image.
        
        Args:
            encoded_payload: Base64-encoded image bytes
            
        Returns:
            JSON string with processed image as base64 and metadata
        """
        try:
            image_bytes = base64.b64decode(encoded_payload)
            processed_bytes, num_faces = blur_faces_in_image(
                image_bytes,
            ...
            processed_payload = base64.b64encode(processed_bytes).decode('utf-8')
            
            return json.dumps({
                "payload": processed_payload,
                "faces_blurred": num_faces > 0,
                "num_faces": num_faces,
                "blur_intensity": self.blur_intensity,
                "detection_settings": {
                    "scale_factor": self.scale_factor,
...
class ObjectRetrievalTool(BaseTool):
    ...
    client: Any = None  # Pydantic field for storage client
    ...
    def _run(self, object_name: str) -> str:
        result = self.client.get_object(object_name)
...
class ImageCaptionTool(BaseTool):
    ...
    client: Any = None  # Pydantic field for vision client
    ...
    def _run(self, encoded_payload: str) -> str:
        image_bytes = base64.b64decode(encoded_payload)
        caption = self.client.generate_caption(image_bytes)
        return caption
...
class DamageDetectionTool(BaseTool):
    ...
    def _run(self, encoded_payload: str) -> str:
        image_bytes = base64.b64decode(encoded_payload)
        result = self.client.detect_damage(image_bytes)
...
def toolset(config: WorkflowConfig) -> Dict[str, BaseTool]:
    ...
    return {
        ...
        "face_blur": FaceBlurringTool(
            blur_intensity=config.privacy.blur_intensity,
            scale_factor=config.privacy.face_detection_scale_factor,
            min_neighbors=config.privacy.face_detection_min_neighbors,
            min_face_size=config.privacy.face_detection_min_size
        ),
